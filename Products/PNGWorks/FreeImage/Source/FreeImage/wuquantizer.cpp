...
//            and adaptation for the FreeImage Library
...
WuQuantizer::WuQuantizer(void *dib)
{
...
float WuQuantizer::Var(Box *cube)
{
	float dr, dg, db, xx;
    ...
    xx =  gm2[INDEX(cube->r1, cube->g1, cube->b1)] 
...
float WuQuantizer::Maximize(Box *cube, uint8 dir, int first, int last , int *cut,
							int32 whole_r, int32 whole_g, int32 whole_b, int32 whole_w)
{
...
bool WuQuantizer::Cut(Box *set1, Box *set2)
{
	uint8 dir;
	int cutr, cutg, cutb;
...
void WuQuantizer::Mark(Box *cube, int label, uint8 *tag)
{
	int r, g, b;
...
void* WuQuantizer::Quantize()
{
	...
	BYTE *new_dib = FreeImage_Allocate(width, height, 8);
	if(!new_dib)
		throw "Not enough memory";
...
void *
FreeImage_ColorQuantize(void *dib) {

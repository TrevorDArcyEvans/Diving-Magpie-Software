...
//  Description: Implements the CJPGWorksApp class which is the
...
//
CJPGWorksApp::CJPGWorksApp(LPSLDWORKS pSldWorks)
{
    // application
...
bool CJPGWorksApp::StartApplication(void)
{
...
//------------------------------------------------------------------------
void CJPGWorksApp::TerminateApplication(void)
{
...
//  
void CJPGWorksApp::AddMenus(void)
{
    HRESULT                     res;
...
//
void CJPGWorksApp::docCreateMenu(long DocumentType)
{
    HRESULT                     hres;
    VARIANT_BOOL                bres;
    ...
    hres = m_pSldWorks->AddFileSaveAsItem(auT(JPG_EXPORT_SAVE_AS),
                    auT(JPG_SAVE_AS_DESCRIP), DocumentType, &bres);


    hres = m_pSldWorks->IFrameObject(&pFrame);

    // add JPGWorks menu
    hres = pFrame->AddMenu(auT(JPG_MENU), swLastPosition, &bres);
    ...
    hres = pFrame->AddMenuItem(
                auT(JPG_MENU), auT(JPG_MENU_ITEM_CONFIGURE), swLastPosition,
                auT(JPG_EXPORT_CONFIGURE),
                &bres);

    // add separator
    hres = pFrame->AddMenuItem(
                    auT(JPG_MENU), NULL, swLastPosition,
                    NULL,
                    &bres);

    // add Help menu item
    hres = pFrame->AddMenuItem(
                auT(JPG_MENU), auT(JPG_MENU_ITEM_HELP), swLastPosition,
                auT(JPG_EXPORT_HELP),
                &bres);

    // add About menu item
    hres = pFrame->AddMenuItem(
                auT(JPG_MENU), auT(JPG_MENU_ITEM_ABOUT), swLastPosition,
                auT(JPG_EXPORT_ABOUT),
                &bres);
...
//------------------------------------------------------------------------
void CJPGWorksApp::RemoveMenus(void)
{
...
//
void CJPGWorksApp::docRemoveMenu(long DocumentType)
{
    HRESULT                     hres;
    VARIANT_BOOL                bres;

    // remove save-as item
    hres = m_pSldWorks->RemoveFileSaveAsItem(auT(JPG_EXPORT_SAVE_AS),
                    auT(JPG_SAVE_AS_DESCRIP), DocumentType, &bres);

    //remove menu
    hres = m_pSldWorks->RemoveMenu(DocumentType, auT(JPG_MENU), NULL, &bres);
}
...
void CJPGWorksApp::AddEvent(CObject* swEventHandler)
{
    // add to the object list
    m_EventList.AddTail(swEventHandler);
}
//------------------------------------------------------------------------
bool CJPGWorksApp::RemoveEvent(CObject* swEventHandler)
{
    POSITION pos;
...
void CJPGWorksApp::ChangeActiveDoc(void)
{
...
//------------------------------------------------------------------------
void CJPGWorksApp::DestroyDoc(LPMODELDOC DocBeingDestroyed)
{
...
//------------------------------------------------------------------------
void CJPGWorksApp::SetSWApp(LPSLDWORKS pSldWorks)
{
    m_pSldWorks = pSldWorks;
}
//------------------------------------------------------------------------
LPSLDWORKS CJPGWorksApp::GetSWApp(void)
{
    return m_pSldWorks;
}
//------------------------------------------------------------------------

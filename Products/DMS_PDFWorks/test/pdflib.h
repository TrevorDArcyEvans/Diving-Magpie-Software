...
#ifdef PDFLIB_EXPORTS        /* prepare a DLL (PDFlib-internal use only) */
    #define PDFLIB_API    __declspec(dllexport)
#elif defined(PDFLIB_STATIC)
    #define PDFLIB_API
#else    /* !PDFLIB_STATIC */    /* PDFlib clients - import the PDFlib fcts. */
...
#define PDFLIB_VERSIONSTRING    "3.0"
...
/* Obsolete, use PDF_get_parameter(p, "fontname", 0) instead. */
PDFLIB_API const char * PDFLIB_CALL
PDF_get_fontname(PDF *p);

/* Obsolete, use PDF_get_value(p, "fontsize", 0) instead. */
PDFLIB_API float PDFLIB_CALL
PDF_get_fontsize(PDF *p);

/* Obsolete, use PDF_get_value(p, "font", 0) instead. */
PDFLIB_API int PDFLIB_CALL
PDF_get_font(PDF *p);
...
/* Format text in the current font and size into the supplied text box
 ...
 the box. */
PDFLIB_API int PDFLIB_CALL
PDF_show_boxed(PDF *p, const char *text, float left, float top, float width, float height, const char *hmode, const char *reserved);

/* Set the distance between text baselines. */
PDFLIB_API void PDFLIB_CALL
PDF_set_leading(PDF *p, float leading);
...
/* Return the width of an image. Obsolete, use
 PDF_get_value(p, "imagewidth", image) instead. */
PDFLIB_API int PDFLIB_CALL
PDF_get_image_width(PDF *p, int image);

/* Return the height of an image. Obsolete, use
 PDF_get_value(p, "imageheight", image) instead. */
PDFLIB_API int PDFLIB_CALL
PDF_get_image_height(PDF *p, int image);
...
#ifndef SWIG
/* Obsolete functions -- use PDF_open_image_file() instead. */

...
// start as invalid cookie
    GlobalInterfacePointer( DWORD Cookie = 0 ) : m_dwCookie( Cookie ) { }
...
// register an interface pointer in GIT
    HRESULT Globalize(Itf *pItf) {
        ...
        HRESULT hr = g_pGIT->RegisterInterfaceInGlobal(pItf, __uuidof(Itf), &m_dwCookie);
        return hr ;
    }
...
// get a local interface pointer from GIT
    HRESULT Localize(Itf **ppItf) const {
        ...
        HRESULT hr = g_pGIT->GetInterfaceFromGlobal(m_dwCookie, __uuidof(Itf),(void**)ppItf);
        return hr ;
    }
// convenience methods
...
//  Detaches the Cookie and Prevents Destruction of the GIP

    DWORD Detach() { DWORD Result = m_dwCookie ;
                     m_dwCookie    = 0 ;
                     return Result ;
                    }
...
#define GIP(Itf) GlobalInterfacePointer<Itf>
...
public:
// import from GIP
    LocalInterfacePointer(const GlobalInterfacePointer<Itf> & rhs, 
                          HRESULT& hr) {
...
// import from raw cookie
    ...
    {
        assert(g_pGIT != 0 && "GIT::Init not called");
        hr = g_pGIT->GetInterfaceFromGlobal(dwCookie, __uuidof(Itf), (void**)&m_pItf);
    }
...
#define LIP(Itf) LocalInterfacePointer<Itf>

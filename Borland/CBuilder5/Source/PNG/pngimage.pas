...
type
  ...
  public
    ...
    function IndexOf(Item: TChunkType): Integer; overload;
    function IndexOf(Item: TChunkClass): Integer; overload;
  ...
  public
    ...
    function Add(Item: TChunk): TChunk; overload;
    function Add(ChunkClass: TChunkClass): TChunk; overload;
    function Add(Stream: TStream): TChunk; overload;
    procedure Remove(Item: TChunk);
    function IndexOf(Chunk: TChunk): Integer; overload;
    function IndexOf(ChunkClass: TChunkClass): Integer; overload;
    procedure Clear;
  end;
...
{:Convert between bit formats}
procedure RowConvertIndexed8(Source: array of Pointer; Target: Pointer;
  Count: Cardinal; Mask: Byte; FSourceBPS, FTargetBPS: Byte);

// This is the core convertion routine for indexed pixel formats.
  ...
  CASE IHDR.ColorType of
    ...
    CASE IHDR.BitDepth of
      2: {2 bits per pixel, not supported by TBitmap, so move to 4 bits}
        RowConvertIndexed8([@Data[0]], ImageData, IHDR.Width, PassMask[Pass], 2, 4);
      4: {4 bits per pixel}
        RowConvertIndexed8([@Data[0]], ImageData, IHDR.Width, PassMask[Pass], 4, 4);
      1: {1 bit per pixel}
        RowConvertIndexed8([@Data[0]], ImageData, IHDR.Width, PassMask[Pass], 1, 1);
      8: {1 byte per pixel}
      repeat
  ...
  case IHDR.ColorType of
    ...
    CASE IHDR.BitDepth of
      ...
      2: {Pixelformat pf2bits ? not supported (pf4bits being used) }
        RowConvertIndexed8([@Data[0]], ImageData, Bitmap.Width, $FF, 2, 4);
...
begin
 ...
 {Create grayscale palette}
  ...
  BEGIN
    ...
    PlteIndex := Owner.IndexOf(TChunkPLTE);
    if PlteIndex <> -1 then
...
function TChunkClasses.IndexOf(Item: TChunkClass): Integer;
var
  i: Integer;
begin
...
function TChunkClasses.IndexOf(Item: TChunkType): Integer;
var
  i: Integer;
begin
  {If none found, return -1}
...
begin
  ...
  {Default class name}
  ClassPos := ChunkClasses.IndexOf(TChunkClass(ClassType));
  if ClassPos <> -1 then
...
function TChunkList.Add(Item: TChunk): TChunk;
begin
...
function TChunkList.IndexOf(ChunkClass: TChunkClass): Integer;
var
  I: Integer;
begin
...
function TChunkList.IndexOf(Chunk: TChunk): Integer;
begin
...
function TChunkList.Add(ChunkClass: TChunkClass): TChunk;
begin
  Result := Add(ChunkClass.Create(Self));
end;

{:Add a chunk to the list when the chunk data needs to be readed
 from a stream.                                                }
function TChunkList.Add(Stream: TStream): TChunk;
var
  CLength: Cardinal;
  CType  : TChunkType;
  CCRC   : Cardinal;
...
begin
  ...
  begin
    ...
    begin
      j := Chunkclasses.IndexOf(SourcePNG.Chunks[i].fType);
      {If the class is a know class, create it using that class}
      {otherwise with the default TChunk class}
      if j <> -1 then
        Chunks.Add(Chunkclasses[j].ChunkClass.Create(Chunks)).Assign(SourcePNG.Chunks[i])
      else
        Chunks.Add(TChunk.Create(Chunks)).Assign(SourcePNG.Chunks[i]);
    end;
...
begin
  ...
  with TChunkIHDR(Chunks.Add(TChunkIHDR)) do
  begin
    Width := 0;
    Height := 0;
    BitDepth := 2;
...
begin
  ...
  begin
    ...
    if (Chunks.Count = 0) or (not (Chunks[0] is TChunkIHDR)) then
      Chunks.Move(Chunks.Add(TChunkIHDR).Index, 0);

    {PLTE chunk needed}
    if ((PixelFormat = pf1bit) or (PixelFormat = pf4bit) or
      (PixelFormat = pf8bit)) and (Chunks.IndexOf(TChunkPLTE) = -1) then
      Chunks.Move(Chunks.Add(TChunkPLTE).Index, 1);

    {If the image needs TRNS chunk}
    if Transparent then
      Chunks.Move(Chunks.Add(TChunkTRNS).Index, 1)
    {If transparency is not being used, delete the transparency chunk(s)}
    else if Chunks.IndexOf(TChunkTRNS) <> -1 then
      repeat
        Chunks.Remove(Chunks[Chunks.IndexOf(TChunkTRNS)]);
      until Chunks.IndexOf(TChunkTRNS) = -1;

    {Make sures that there is a IEND chunk}
    if Chunks.IndexOf(TChunkIEND) = -1 then
      Chunks.Add(TChunkIEND);

    {Make sures that there is a IDAT chunk}
    if Chunks.IndexOf(TChunkIDAT) = -1 then
      Chunks.Move(Chunks.Add(TChunkIDAT).Index, 1);
...
begin
  ...
  begin
    ...
    {Read the chunks}
    while (not (Chunks.Add(Stream) is TChunkIEND)) and
    ...
    if Chunks.IndexOf(TChunkTRNS) <> -1 then
    case TChunkIHDR(Chunks[0]).ColorType of
      RGB:
      begin
        TransparentColor :=
          TChunkTRNS(Chunks[Chunks.IndexOf(TChunkTRNS)]).GetRGBColor;
        Transparent := TRUE;
      end;
    end;
...
begin
  result := update_crc(chunktype, $ffffffff, buf, len) xor $ffffffff;
end;

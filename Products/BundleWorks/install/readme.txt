...
1) The following files are required:
    Windows NT:
	BunWksNT.dll		BundleWorks
	BunWksNT.reg		registry file
	IsUn32ex.exe            uninstallation program

    Windows 95:
	BunWks95.dll		BundleWorks
	BunWks95.reg		registry file
	IsUn32ex.exe            uninstallation program

    Windows NT & 95 system:
	BunWks.hlp		help file
	BunWks.dll		BundleWorks support file
	unzdll.dll		unzip dll
	zipdll.dll		zip dll

2) Place all the BundleWorks files, except for
the system files, in the:

    C:\Program Files\SolidPartners\BundleWorks

directory.  If this directory does not exist, create it.

Place the sysem files in your Windows system directory, 
usually C:\WinNT\system32 (Windows NT) or 
C:\Windows\system (Windows 95).
